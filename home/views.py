...
from .forms import PostCreateUpdateForm, CommentCreteForm, CommentReplyForm, PostSearchForm
...
class PostAddReplyView(LoginRequiredMixin, View):
    ...
    def post(self, request, post_id, comment_id):
        ...
        if form.is_valid():
            ...
            reply.is_reply = True
            reply.save()
            messages.success(request, ' yout reply submited success' 'success')
        return redirect('home:post_detail', post.id, post.slug)
...
class PostLikeView(LoginRequiredMixin, View):
    def get(self, request, post_id):
        post = get_object_or_404(Post, id=post_id)
        like = Vote.objects.filter(post=post_id, user=request.user)
        if like.exists():
